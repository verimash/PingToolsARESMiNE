...
public class PingAndAddMirror {
    public static void execute() {
        ...
        System.out.println("Начинаю выбирать лучший! Знайте, я очищу hosts для того, чтобы не было повторений.");
        try {
            // Создание bat-файла
            ...
            // Запуск bat-файла
            System.out.println("Удаляю из hosts. Подтвердите права Администратора.");
            ...
            System.out.println("Произошла ошибка при создании или удаления из файла: " + e.getMessage());
        ...
        if (bestIp != null) {
            System.out.println("\nСамый лучший IP: " + bestIp + ", с задержкой " + bestTime + " мс");
            System.out.println("Добавляем изменения в hosts!");
            try {
                // Создание bat-файла
                String filename = generateFileName();
                File batFile = new File(filename + ".bat");
                ...
                bufferedWriter.write("@echo off\n" +
                        ...
                        "echo " + bestIp + " mc.aresmine.ru >> %hostspath%\n" +
                        "echo " + bestIp + " mc.aresmine.me >> %hostspath%\n" +
                        "echo " + bestIp + " play.aresmine.me >> %hostspath%\n" +
                        "echo " + bestIp + " hot.aresmine.me >> %hostspath%\n");
                bufferedWriter.close();

                // Запуск bat-файла
                System.out.println("Добавляю в hosts. Подтвердите права Администратора.");
                ProcessBuilder processBuilder = new ProcessBuilder("cmd.exe", "/c", filename+".bat");
                processBuilder.directory(new File(System.getProperty("user.dir")));
                Process process = processBuilder.start();
                ...
                // Ждем завершения процесса
                int exitCode = process.waitFor();
                System.out.println("\n Самый лучший сервер успешно указан! Если на данный момент Вы находитесь на ARESMiNE, перезайдите!\n");
            } catch (IOException e) {
                System.out.println("Произошла ошибка при создании или записи в файл: " + e.getMessage());
            } catch (InterruptedException e) {}
        } else {
